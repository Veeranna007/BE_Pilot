...
getfilterPC.get('/getfilterPC', async (req, res) => {
  ...
  let userEmail = userDetails.userEmail;
    ...
    let { status, employeeId, select_date, week_no, selectStart_daterange, selectEnd_daterange } = req.query;
    ...
    try {
      ...
      infoLogger.log("info", `Username: ${userName} from company: ${companyUrl}  hit the getfilterPC with query params: ${JSON.stringify(req.query)}`)
      ...
      const query = {'fmsProcessID.processCoordinatorId':processCoordinatorId};
    ...
    // Handle date range
      ...
      if (week_no) {
        try {
          console.log("Received week_no:", week_no);
           ...
           // Fetch company starting day of the week uat
         ...
         const companyStartingDayWeekResponse = await instanceStartingDayWeekResponse.post(
          ...
          if (matchingWeek) {
            ...
            errorLogger.log("error" , `User:${userName} from company: ${companyUrl} provided invalid week no:${week_no}`);
      ...
      infoLogger.log("info", `Username: ${userName} from company:${companyUrl} successfully fetch the filter PC fms data ${JSON.stringify(taskDocuments)}`)
      ...
      errorLogger.log("error" , `Username:${userName} from company:${companyUrl} failed to fetch the filter PC fms data due to ${error.message}`);
