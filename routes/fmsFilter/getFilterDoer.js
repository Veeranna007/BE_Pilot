...
getfilterDoer.get('/getfilterDoer', async (req, res) => {
  ...
  let { status, processId, select_date, week_no, selectStart_daterange, selectEnd_daterange } = req.query;
  ...
  try {
    ...
    infoLogger.log("info", `Username: ${userName} from company: ${companyUrl}  hit the getfilterDoer with query params: ${JSON.stringify(req.query)}`)
    ...
    infoLogger.log("info", `Username: ${userName} from company:${companyUrl} successfully fetch the filter doer fms data ${JSON.stringify(taskDocuments)}`)
    ...
    errorLogger.log("error" , `Username:${userName} from company:${companyUrl}  failed to fetch the filter doer fms data due to ${error.message}`);
