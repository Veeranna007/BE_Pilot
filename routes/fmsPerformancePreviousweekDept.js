...
const fmsPerformancePreviousweekDept = express.Router();
...
fmsPerformancePreviousweekDept.post("/fmsPerformancePreviousweekDept", async (req, res) => {
    ...
    try {
        ...
        infoLogger.log("info", `company:${companyUrl} hit the api fmsPerformancePreviousweekDept with body params: ${JSON.stringify(req.body)}`)
        ...
        infoLogger.log("info", `company:${companyUrl} successfully fetch the performance for previous week of requested department:
              week_no: ${weekNo},
            TotalTask: ${TotalTask},
        ...
        errorLogger.log("error", `company:${companyUrl} failed to fetch the performance for previous week of requested department due to ${error.message}`);
       return res.status(500).send({ error: error, status: 500 });
    }
});

module.exports = fmsPerformancePreviousweekDept;
